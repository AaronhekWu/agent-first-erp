-- 0018 — 老带新收敛到优惠组合
--
-- 取消独立的"老带新"报名流程: 报名时只保留「正常 / 优惠 / 自定义」。
-- 老带新作为 referral 类型的优惠组合, 通过 p_source='campaign' 直接选用 (不再强制填推荐人)。
-- 仅放宽 campaign 分支的类型限制; 保留 p_source='referral' 分支以兼容历史调用。
-- 签名不变, CREATE OR REPLACE 即可。
...
begin
  ...
  if p_campaign_id is not null then
    select * into v_campaign from public.promo_campaigns
     ...
     for update;
    if not found then raise exception '优惠组合无效、不适用于本课程或已过期'; end if;
  ...
  if p_source = 'normal' and (p_campaign_id is not null or p_custom_discount_type is not null) then
    ...
    raise exception '优惠报名必须选择有效的优惠组合';
